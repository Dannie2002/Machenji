...
: Events_data.filter(event => event.Category === selectedCategory);
  ...
  return (
    ...
    <div className="Section_wrapper relative z-50">
       ...
       <div className="flex  flex-col mt-12  items-start  justify-center space-y-6 ">
         <div className='bg-(--primary-color) px-4 py-2'>
      ...
      transition={{ duration: 0.15, ease: "easeOut" }}
         className="text-[24px] z-50 leading-[28px] text-left  text-grey  zalando Capitalize font-light  mt-2  lg:text-[18px] lg:leading-[22px]  lg:max-w-full"> <span className='text-[#fffced]'>Find amazing</span> events</motion.h1>
      ...
      />       
        ...
        <motion.div
         ...
         transition={{ duration: 0.15, ease: "easeOut"}}
         className="Grid_4 lg:grid-cols-4 gap-6" >
       {filteredEvents.map((event) => (
       <div key={event.id} className="group rounded-sm border cursor-pointer hover:bg-[#272626] border-(--text-color)/40 flex flex-col  gap-2 lg:gap-0 overflow-x-hidden relative">
        

        <div className="lg:h-[220px] h-[220px] relative">
          <img
            src={event.image}
            alt={event.title}
          ...
          />
          <div className='hidden transition-all duration-470 gap-4 group-hover:flex absolute z-20 bottom-4 left-4 px-4 py-2 bg-black/50 text-white rounded-full'>
        ...
        <motion.div 
           variants={containerVariants}
            ...
            whileInView="show"
            viewport={{once:true}}
         className="flex relative items-start flex-col p-4 lg:p-4 gap-2  justify-start z-50">
          ...
          <motion.h4 variants={itemVariants} className="text-grey text-[16px] zalando uppercase font-bold capitalize lg:text-[20px] z-50">
            ...
            <p className='  font-light text-grey capitalize zalando text-[12px]'>Sponsored by <span className='font-semibold'>Landrord Entertainment</span></p>
                ...
                <motion.div className='flex items-center mt-4 py-2 gap-2 group relative px-4 cursor-pointer rounded-sm  text-[#fffced] chivo uppercase font-bold transition-all duration-300 bg-(--primary-color) border border-none  '>
